...
var displayItem = function(state, element){
	var htmlItems = state.items.map(function(item){
		return '<li class="box" id="listBox">' + item + '</br><button class="divBtns" id="checkBtn">Check</button><button class="divBtns" id="deleteBtn">Delete</button>' + '</li>'; 
	});
...
*/
	...
	$(document).ready(function(e){ 				
			...
			$('ul').on('click', '#deleteBtn', function(event){
				...
				state.items.splice(index,1); //NEW
				$(this).closest("li").remove();
			});

			$('ul').on('click', '#checkBtn', function(event){
